...
void createCoins()
{
    ...
    if(bFirstTime) {
        ...
        gfx = createGfxObject(  "../coin-40st.png" ); 
    }
       
    init(&coin[0], &gfx, (vec2f){400, 100},  0);
    init(&coin[1], &gfx, (vec2f){500, 100}, 10);
    init(&coin[2], &gfx, (vec2f){600, 100}, 20);
    init(&coin[3], &gfx, (vec2f){700, 100}, 30);
    gameObjects[nGameObjects++] = &coin[0];
    gameObjects[nGameObjects++] = &coin[1];
    gameObjects[nGameObjects++] = &coin[2];
    gameObjects[nGameObjects++] = &coin[3];
}
