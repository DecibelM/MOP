...
static void update(GameObject* this)
{
   ...
   if(this->pos.x <= this->obj.outputWidth/2 && this->moveDir.x < 0) {
       int overflow = (this->obj.outputWidth/2) - this->pos.x;
       this->pos.x += 2*overflow;
       this->moveDir.x = -this->moveDir.x;
   }
   if(this->pos.x >= screenWidth - this->obj.outputWidth/2 && this->moveDir.x > 0) 
   {
       int overflow = this->pos.x - (screenWidth - this->obj.outputWidth/2);
       this->pos.x -= 2*overflow;
       this->moveDir.x = -this->moveDir.x;
   }
   
   if(this->pos.y <= this->obj.outputHeight/2 && this->moveDir.y < 0) {
      int overflow = (this->obj.outputHeight/2) - this->pos.y;
       this->pos.y += 2*overflow;
       this->moveDir.y = -this->moveDir.y;
   }
   if(this->pos.y >= screenHeight - this->obj.outputHeight/2 && this->moveDir.y > 0)
   {
       int overflow = this->pos.y - (screenHeight - this->obj.outputHeight/2);
       this->pos.y -= 2*overflow;
       this->moveDir.y = -this->moveDir.y;
   }    
}
...
{
    pObj->obj = *pGfx;
    pObj->obj.outputWidth  = w;
    pObj->obj.outputHeight = h;
    pObj->pos = pos;
